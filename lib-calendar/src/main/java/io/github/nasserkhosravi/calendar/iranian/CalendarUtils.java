...
public class CalendarUtils {
    ...
    public static long persianToJulian(long year, int month, int day) {
        return 365L * ((ceil(year - 474L, 2820D) + 474L) - 1L) + ((long) Math.floor((682L * (ceil(year - 474L, 2820D) + 474L) - 110L) / 2816D)) + (PersianCalendar.PERSIAN_EPOCH - 1L) + 1029983L
    ...
    public static boolean isPersianLeapYear(int persianYear) {
        return CalendarUtils.ceil((38D + (CalendarUtils.ceil(persianYear - 474L, 2820L) + 474L)) * 682D, 2816D) < 682L;
    }
     ...
     */
    public static BaseCalendar trimToMidnight(BaseCalendar calendar) {
    ...
    public static HashSet<BaseCalendar> getDatesBetween(BaseCalendar finishDate, BaseCalendar startDate) {
        HashSet<BaseCalendar> array = new HashSet<>();
        BaseCalendar start = startDate;
        BaseCalendar temp;

        while (finishDate.isAfterDate(new YearMonthDay(start.getYear(), start.getMonth(), start.getDay()))) {

            array.add(start);
            temp = start;
            start = finishDate.createNewInstance();
            start.setDate(temp.getYear(), temp.getMonth(), temp.getDay() + 1);
    ...
    public static ArrayList<BaseCalendar> getDatesBetweenInArrayList(BaseCalendar endDate, BaseCalendar startDate) {
        ArrayList<BaseCalendar> array = new ArrayList<>();
        BaseCalendar start = startDate;
        BaseCalendar temp;

        while (endDate.isAfterDate(new YearMonthDay(start.getYear(), start.getMonth(), start.getDay()))) {

            array.add(start);
            temp = start;
            start = endDate.createNewInstance();
            start.setDate(temp.getYear(), temp.getMonth(), temp.getDay() + 1);
    ...
    public static ArrayList<BaseCalendar> getYearsBetweenInArrayList(BaseCalendar endDate, BaseCalendar startDate) {
        ArrayList<BaseCalendar> array = new ArrayList<>();
        BaseCalendar start = startDate;
        while (endDate.getYear() > start.getYear()) {
            array.add(start);
            start = start.plusYear(1);
        }
        array.add(endDate);
        return array;
    }
